 ...
 * Name(s): David Gries, Nishi Kantamneni
 * What I thought about this assignment:
 * I thought that this assignment was really fun because it was based on a
 * real life situation and that made it more interesting to solve.
 *  It was also relatively easy to solve.
...
public class FluTree {
    ...
    public FluTree insert(Person c, Person p) throws IllegalArgumentException {
        ...
        if (p == null || c == null || contains(c) ||
            node(p) == null) { throw new IllegalArgumentException(); }
        node(p).children.add(new FluTree(c));
        return node(c);
    }
    ...
    public int depth(Person p) {
        ...
        if (node(p) == null) return -1;
        if (rootPerson() == p) return 0;
        int d= 0;
        for (FluTree ft : children) {
            d= d + 1 + ft.depth(p);
        }
    ...
    public List<Person> fluRouteTo(Person c) {
        ...
        if (rootPerson() == c) {
            LinkedList<Person> route= new LinkedList<>();
            route.addFirst(c);
            return route;
        }
        for (FluTree ft : children) {
            List<Person> L= ft.fluRouteTo(c);
            if (L != null) {
                L.add(0, rootPerson());
                return L;
            }
    ...
    public Person commonAncestor(Person child1, Person child2) {
        ...
        if (child1 == null || child2 == null || !contains(child1) ||
            !contains(child2)) { return null; }
        List<Person> c1= fluRouteTo(child1);
        List<Person> c2= fluRouteTo(child2);
        for (int i= c1.size() - 1; i != -1; i-- ) {
            Person per= c1.get(i);
            if (c2.contains(per)) return per;
        }
        return null;
    }
    ...
    public boolean equals(Object ob) {
        ...
        FluTree fts= (FluTree) ob;
        if (rootPerson() != fts.rootPerson()) return false;
        if (size() != fts.size()) return false;
        for (FluTree ft : children) {
            if (!ft.help(ft, fts.children)) return false;
        }
        return true;
    }
